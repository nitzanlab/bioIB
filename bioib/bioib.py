...
class bioIB:
    ...
    def annotate_adata(self, n_leaves=None, initial_annotation=True):
        ...
        for mtg in range(n_leaves):
            ...
            adata.obs['Metagene %s' % mtg] = average_vector
        ...
        adata.obsm['bioIB_compressed_data'] = average_mat
        # adding a metagene-gene mapping and markers
        adata.uns['bioIB_gene_MG_mapping'] = x_given_xHat
        ...
        adata.uns['bioIB_MG_group_of_interest_mapping'] = y_given_xHat
    ...
    def flat_clustering(self, n_metagenes, n_iters=3):
        ...
        for mtg in range(n_metagenes):
            ...
            average_vector = np.sum(multiplied_by_factors, axis=1)
            average_mat[:, mtg] = average_vector.reshape(-1, )
            self.adata.obs['Metagene %s' % mtg] = average_vector
            top_genes = self.adata.var_names[np.argsort(factors)[::-1][:self.n_markers]]
            marker_table.append(list(top_genes))

        self.adata.uns['bioIB_flat_%s_MGs_gene_to_MG' % n_metagenes] = all_x_given_xhat[max_Ity]
        self.adata.uns['bioIB_flat_%s_MGs_group_of_interest_to_MG' % n_metagenes] = all_y_given_xhat[max_Ity]
        # creating a new layer in adata.obsm to contain the compressed adata
        self.adata.obsm['bioIB_compressed_data_flat_%s_MGs' % n_metagenes] = average_mat
        # adding the marker list
